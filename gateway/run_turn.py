...
from gateway.platforms.base import BasePlatformAdapter, ProcessingOutcome, TurnDeliveryPreparationError
...
class GatewayTurnMixin:
    ...
    def _proxy_stream_consumer(self, source: "SessionSource", event_message_id, _thread_metadata, _run_still_current):
        ...
        _adapter = self._delivery_adapter_for(source)
        if not (_scfg.enabled_for(_plat_streaming) and _adapter is not None
                and getattr(_adapter, "supports_response_streaming", True)):
            return None
        try:
