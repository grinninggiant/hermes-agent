...
export async function bundleElectronMain({ source, out, stamp, dev = false }) {
  ...
  // Offline maintenance remains independent of Electron startup and tsx.
  // Use the same prepared source/compiler/output, not the caller's checkout.
  for (const name of ['bundle-offline', 'runtime-offline']) {
    await build({
      ...common,
      entryPoints: [join(source, `apps/desktop/scripts/${name}.ts`)],
      format: 'esm',
