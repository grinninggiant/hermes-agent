...
async function checkUpdates(opts: { force?: boolean } = {}): Promise<UpdaterStatusWire> {
  return startupEffects.checkForUpdates(() => checkUpdatesExternal(opts))
}

async function checkUpdatesExternal(opts: { force?: boolean } = {}): Promise<UpdaterStatusWire> {
...
async function resolveHermesBackend(backendArgs: string[], runtimeProfile?: string): Promise<ResolvedHermesBackend> {
  // Exact General selections take precedence over bundled and discovered runtimes.
  // A failed validation must not silently start a different runtime.
  if (runtimeProfile === 'general') {
    ...
    if (selected) {
      ...
      // The validated venv interpreter owns its site-packages. The source helper
      // scrubs inherited Python paths instead of mixing in another environment.
      const backend = createSourcePythonBackend(coordinate.agentRoot, coordinate.python, backendArgs)!

      return { ...backend, label: `Hermes release ${coordinate.commit}` }
    }
  }
...
function profileRouteOptions(
  ...
  return {
    ...
    exactLocalRuntime:
      (key === 'general' || primaryProfileKey() === 'general') &&
      Boolean(readDesktopConnectionsRegistry().connections.find(c => c.id === 'local')?.generalRuntime?.coordinate),
...
async function ensureBackendExternal(
  profile: string | null | undefined,
  opts: Parameters<typeof ensureBackend>[1] = {}
): Promise<Awaited<ReturnType<typeof backendConnectionState.getPromise>>> {
...
async function runPoolBackendStart(
  ...
  const backend = await ensureRuntime(await resolveHermesBackend(backendArgs, profile), () =>
    assertPoolEntryStillOwned(poolKey, entry, backendPool, localBackendLifecycle.signal)
  )
...
> {
  ...
  const start: Promise<Awaited<ReturnType<typeof backendConnectionState.getPromise>>> = startupEffects.startBackend(() =>
    ...
    const setup = await runPrimaryBackendStartup({
      ...
      attachHostBackend: () =>
        activeProfile === 'general'
          ? nativeRuntimeTransition.attachHostBackend(attachToRunningHostBackend)
