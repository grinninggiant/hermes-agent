...
export async function isIndependentGeneralService(row: { pid: number; command: string }): Promise<boolean> {
  ...
  const readPython = () => {
    ...
    if (!path.isAbsolute(python) || /\s/.test(python)) {throw new Error('Unknown service interpreter')}
  ...
  const readJob = async () => {
    ...
    if (!text.startsWith(`${target} = {\n`) || !text.endsWith('}')) {throw new Error('Unknown job')}
    ...
    const field = (key: string) => {
      ...
      if (values.length !== 1) {throw new Error('Incomplete job identity')}

      return values[0][1]
    }

    const identity = {
      program: field('program'), pid: field('pid'), state: field('state'),
      type: field('type'), path: field('path'), runs: field('runs')
    }

    if (identity.program !== path.join(home, '.hermes/scripts/hermes-serve-keychain.sh') ||
        identity.path !== path.join(home, 'Library/LaunchAgents/ai.hermes.serve-general.plist') ||
        identity.type !== 'LaunchAgent' || identity.state !== 'running' ||
        identity.pid !== String(row.pid) || !/^[1-9]\d*$/.test(identity.runs)) {
  ...
  const readProcess = async (python: string) => {
    ...
    if (!match || executable !== python || match[2] !== row.command) {throw new Error('Unknown process incarnation')}
  ...
  try {
    ...
    if (row.command !== `${python} ${launcher} --profile general serve --isolated --host 127.0.0.1 --port 9120`) {return false}
