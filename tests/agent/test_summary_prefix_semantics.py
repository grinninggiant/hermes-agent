...
_FROZEN_PREFIX_GENERATIONS = (
    # Immediately before contextual memory priority; exact persisted wire text.
    ...
    " resume, wrap up, or continue work from '## Historical Task Snapshot' or any other "
    "section, do not call tools, and wait for a new user message. This handoff must never "
    "become the active turn by itself. (Exception: if tool results or your own tool calls "
    "appear after this summary, you are mid-way through an in-flight exchange — continue that"
    " exchange normally.) Topic overlap with the summary does NOT mean you should resume its "
    "task: even on similar topics, the latest user message WINS. Treat ONLY the latest "
    "message as the active task and discard stale items from '## Historical Task Snapshot' "
    "entirely — do not 'wrap up' or 'finish' work described there unless the latest message "
    "explicitly asks for it. Reverse signals in the latest message (e.g. 'stop', 'undo', "
    "'roll back', 'just verify', 'don't do that anymore', 'never mind', a new topic) must "
    "immediately end any in-flight work described in the summary; do not re-surface it in "
    "later turns. IMPORTANT: Your persistent memory (MEMORY.md, USER.md) in the system prompt"
    " is ALWAYS authoritative and active — never ignore or deprioritize memory content due to"
    " this compaction note. None of the above restricts HOW you work: your tools remain fully"
    " active — keep calling them normally for the active task (edit files, run commands, "
...
# regression below. Index 2 after the contextual-memory freeze was prepended.
...
def test_pre_contextual_memory_handoff_normalizes_without_old_directive():
    from agent.context_compressor import ContextCompressor

    old = _FROZEN_PREFIX_GENERATIONS[0] + "\nBODY"
    assert ContextCompressor._is_context_summary_content(old)
    assert ContextCompressor._with_summary_prefix(old) == SUMMARY_PREFIX + "\nBODY"
    assert "ALWAYS authoritative" not in ContextCompressor._with_summary_prefix(old)
